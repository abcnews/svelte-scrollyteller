...
export function setIsSplitScreen([screenDims, globalAlign]) {
    return derived([screenDims, globalAlign], ([$screenDims, $globalAlign]) => ["left", "right"].includes($globalAlign) &&
 ...
 */
export function setMaxGraphicWidth([isSplitScreen, graphicRootDims, screenDims, ratio, maxScrollytellerWidth,]) {
    return derived([isSplitScreen, graphicRootDims, screenDims, ratio, maxScrollytellerWidth], ([$isSplitScreen, $graphicRootDims, $screenDims, $ratio, $maxScrollytellerWidth,]) => {
        if (!$isSplitScreen) {
            return 1e6;
        }
